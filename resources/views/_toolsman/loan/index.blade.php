    ...
    <div class="px-2 pb-4">
        <form action="{{ route('toolsman.loans.index') }}" method="GET" navigate-form
            ...
            <div class="flex gap-x-2">
                ...
                @if (!empty($keywords))
                    <a href="{{ route('toolsman.loans.index') }}"
    ...
    <div class="overflow-x-auto border border-gray-200 rounded-lg dark:border-neutral-700">
        <table class="w-full divide-y divide-gray-200 dark:divide-neutral-700">
            <thead class="bg-gray-50 dark:bg-neutral-800">
                <tr>
                    <th class="px-6 py-3 text-start text-xs font-semibold uppercase text-gray-800 dark:text-neutral-200">ID Peminjaman</th>
                ...
                <tr class="hover:bg-gray-50 dark:hover:bg-neutral-700">
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-800 dark:text-neutral-200">
                        <div class="flex flex-col">
                            ...
                            @if($loan->status === 'returned' && $loan->fine_amount > 0)
                                <span class="text-xs text-gray-500 flex gap-[2px]">Denda Rp.{{ number_format($loan->fine_amount, 0, ',', '.')}} 
                                    <span>({{ $fineStatus }})</span>
                                </span>
                            @endif
                        </div>
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap text-end text-sm font-medium">
                        <div class="flex justify-end items-center gap-x-2">
                            ...
                            <a navigate href="{{ route('toolsman.loans.detail', $loan->id) }}" 
