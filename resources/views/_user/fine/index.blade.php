...
<div class="grid gap-3 md:flex md:justify-between md:items-center py-4">
    <div>
        <h1 class="text-2xl font-extrabold text-gray-800 dark:text-neutral-200 mb-1">
            Manajemen Denda Anda
        </h1>
        <p class="text-md text-gray-400 dark:text-neutral-400">
            Manajemen denda untuk keterlambatan pengembalian barang.
        </p>
    </div>
...
<div class="border-b border-gray-200 dark:border-neutral-700 mb-6">
    <nav class="flex space-x-4 overflow-x-auto no-scrollbar" aria-label="Tabs" role="tablist">
        ...
        <a href="{{ route('user.fines.index', ['fine_status' => 0]) }}"
        ...
        <a href="{{ route('user.fines.index', ['fine_status' => 1]) }}"
...
<div class="flex flex-col">
    ...
    <div class="px-2 pb-4">
        <form action="{{ route('user.fines.index') }}" method="GET" navigate-form
                            ...
                            <div class="flex justify-end items-center gap-x-2">
                                ...
                                <div class="hs-dropdown relative inline-flex">
                                    ...
                                    <div class="hs-dropdown-menu transition-[opacity,margin] duration hs-dropdown-open:opacity-100 opacity-0 hidden min-w-60 bg-white shadow-md rounded-lg p-2 mt-2 dark:bg-neutral-800 dark:border dark:border-neutral-700 dark:divide-neutral-700 z-30" aria-labelledby="hs-dropdown-custom-icon-trigger">
                                        ...
                                        <a href="{{ route('user.fines.paid-report', $fineLoan->id) }}" class="flex items-center gap-x-3 py-2 px-3 rounded-lg text-sm text-gray-800 hover:bg-gray-100 dark:text-neutral-400 dark:hover:bg-neutral-700 dark:hover:text-neutral-300" href="#">
...
<div id="loading-overlay" class="fixed inset-0 z-[9999] hidden flex flex-col items-center justify-center bg-white/60 backdrop-blur-sm dark:bg-neutral-900/60">
    <div class="flex flex-col items-center">
        <div class="animate-spin inline-block size-12 border-[4px] border-blue-600 border-t-transparent rounded-full" role="status">
            <span class="sr-only">Loading...</span>
        </div>
        <p class="mt-4 text-lg font-bold text-gray-800 dark:text-white animate-pulse">
...
<script>
    function payFine(loanId) {
        ...
        // 1. Tampilkan Loading Layar
        overlay.classList.remove('hidden');

        fetch(`/user/fines/pay/${loanId}`)
            .then(response => {
                ...
                if (data.token) {
                    window.snap.pay(data.token, {
                        onSuccess: function(result) {
                            // Tidak perlu sembunyikan overlay karena akan pindah halaman
                            window.location.href = "{{ route('user.fines.index', ['fine_status' => 1]) }}";
                        },
                        ...
                        onError: function(result) {
                            ...
                            overlay.classList.add('hidden'); // Sembunyikan loading jika error
                        },
                        onClose: function() {
                            // SANGAT PENTING: Sembunyikan loading jika user menutup pop-up Midtrans
            ...
            .catch(error => {
                ...
                overlay.classList.add('hidden'); // Sembunyikan loading jika fetch gagal
            });
    }
</script>
