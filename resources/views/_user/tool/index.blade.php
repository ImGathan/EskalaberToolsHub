    ...
    <div class="group flex flex-col h-full bg-white border border-gray-200 rounded-xl dark:bg-neutral-800 dark:border-neutral-700 shadow-lg hover:shadow-xl hover:shadow-blue-500/20 transition-all duration-300 overflow-hidden">
        ...
        <div class="p-5 flex flex-col flex-grow">
            <div class="flex items-center justify-between mb-2">
                <span class="inline-flex items-center py-1 px-2 rounded-lg bg-blue-100 text-blue-700 text-[10px] font-bold uppercase tracking-wider dark:bg-neutral-700 dark:text-blue-400 max-w-[100px] truncate" title="{{ $tool->category->name }}">
                    {{ $tool->category->name }}
                </span>
            ...
            <h2 class="text-lg font-bold text-gray-700 dark:text-neutral-200 group-hover:text-blue-600 transition-colors line-clamp-1" title="{{ $tool->name }}">
                {{ $tool->name }}
            </h2>

            <span class="text-xs text-blue-600 dark:text-blue-400 font-semibold truncate flex-1" title="{{ $tool->place->name }}">
                {{ $tool->type->name }}
            </span>

            <div class="flex items-center gap-x-2 mb-3 pt-2">
                ...
                <span class="text-xs text-gray-600 dark:text-neutral-400 font-medium truncate flex-1" title="{{ $tool->place->name }}">
                    {{ $tool->place->name }}
                </span>
            </div>
            ...
            <div class="mt-auto">
                ...
                @else
                <button class="w-full py-3 px-4 inline-flex justify-center items-center gap-x-2 text-xs font-bold rounded-xl border border-transparent bg-gray-600 text-white cursor-not-allowed">
                    Stok Kosong
                </button>
                @endif
            </div>
