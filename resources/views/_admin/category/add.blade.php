...
<div class="grid grid-cols-1 md:grid-cols-2 gap-4">
    <div
        ...
        <form id="add-form" class="p-6" navigate-form action="{{ route('admin.categories.create') }}" method="POST">
            @csrf
            ...
            <div class="space-y-4">
                ...
                <div>
                    ...
                    <input type="text" id="name" name="name" value="{{ old('name') }}"
                        ...
                        placeholder="Contoh: Alat Elektronik" required>
                    @error('name')
                ...
                {{-- Toolsman --}}
                <div>
                    <label for="toolsman_id" class="block text-sm font-medium mb-2 dark:text-white">Toolsman <span
                            class="text-red-500">*</span></label>
                    <select id="toolsman_id" name="toolsman_id"
                        class="py-3 px-4 block w-full border-gray-200 rounded-lg text-sm focus:border-blue-500 focus:ring-blue-500 disabled:opacity-50 disabled:pointer-events-none dark:bg-neutral-900 dark:border-neutral-700 dark:text-neutral-400 dark:placeholder-neutral-500 dark:focus:ring-neutral-600 @error('toolsman_id') border-red-500 focus:border-red-500 focus:ring-red-500 @enderror"
                        required>
                        <option value="">-- Pilih Toolsman --</option>
                        @foreach ($toolsmen as $toolsman)
                        <option value="{{ $toolsman->id }}"
                            {{ old('toolsman_id') == $toolsman->id ? 'selected' : '' }}>
                            {{ $toolsman->username }} (Class: {{ $toolsman->class ?? '-' }})
                        </option>
                        @endforeach
                    </select>
                    @error('toolsman_id')
            ...
            <div class="mt-4 flex justify-start gap-x-2">
                <a navigate href="{{ route('admin.categories.index') }}"
