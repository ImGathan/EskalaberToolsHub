        ...
        <form action="{{ route('admin.users.index') }}" method="GET" navigate-form
            ...
            <div class="sm:w-48">
                <select name="access_type"
                    class="py-2 px-3 block w-full border-gray-200 rounded-lg text-sm focus:border-blue-500 focus:ring-blue-500 dark:bg-neutral-900 dark:border-neutral-700 dark:text-neutral-400">
                    <option value="all" {{ ($access_type ?? 'all') == 'all' ? 'selected' : '' }}>Semua Hak Akses</option>
                    <option value="1" {{ ($access_type ?? '') == '1' ? 'selected' : '' }}>Admin</option>
                    <option value="2" {{ ($access_type ?? '') == '2' ? 'selected' : '' }}>Toolsman</option>
                    <option value="3" {{ ($access_type ?? '') == '3' ? 'selected' : '' }}>User</option>
                </select>
            </div>
